...
def add_h2_retro(n, baseyear, params):
    ...
    for original_carrier, new_carrier in plant_types:
        ...
        plant_i = n.links.query(f"carrier == '{original_carrier}'")                     #alle links die z.b. OCGT enthalten
        # Further filtering based on build_year excluding the current planning horizon
        plant_i = plant_i.loc[
            (plant_i.build_year >= start) & (plant_i.build_year != baseyear)            #ab 2030 oder so
        ].index
        ...
        plant_i = [index for index in plant_i if index not in h2_counterpart]           #alle powerplants ohne H2-counterpart

        if not plant_i:
            logger.info(f"No more {original_carrier} retrofitting potential.")
            continue

        n.links.loc[plant_i, "p_nom_max"] = n.links.loc[plant_i, "p_nom"]               # dürfen nicht mehr weiter ausgebaut werden

        df = n.links.loc[plant_i].copy()
        # Adjust bus 0
        df["bus0"] = df.bus0.map(n.buses.location) + " H2"                              #transfer von gas bus zu h2 bus
                                                                                        #bus0 ist bei links immder der ursprungsbus
        ...
        df.loc[:, "capital_cost"] += params.retrofit_cost                               #es kann nur rückbauen, wenn es retrofitted
